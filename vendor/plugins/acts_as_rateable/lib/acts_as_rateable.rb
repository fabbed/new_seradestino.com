  ...
  module Acts
    module Rateable
      ...
      module InstanceMethods
        ...
        def ratings_count_top
          return 0 if !rating
          rating.user_ratings.count(:conditions => {:score => 5})
        end

        def ratings_count_flop
          return 0 if !rating          
          rating.user_ratings.count(:conditions => {:score => 1})
        end
        ...
        def ratings_flop
          return 0 if !rating          
          rating.user_ratings.count(:all, :conditions => {:score => 1})
        end
        ...
        # Checks whether a user rated the object or not.
        def rated_by_cookie?(vcode)
          rating && rating.user_ratings.exists?(:vcode => vcode)
        end
